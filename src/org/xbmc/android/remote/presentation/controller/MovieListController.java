...
public class MovieListController extends ListController implements IController {
	...
	public void onCreate(Activity activity, Handler handler, AbsListView list) {
		...
		((ISortableManager) mVideoManager)
				.setSortKey(AbstractManager.PREF_SORT_KEY_MOVIE);
		((ISortableManager) mVideoManager).setPreferences(activity
		...
		if (!isCreated()) {
			...
			mActor = (Actor) mActivity.getIntent().getSerializableExtra(
					ListController.EXTRA_ACTOR);
			...
			mFallbackBitmap = BitmapFactory.decodeResource(
					activity.getResources(), R.drawable.default_poster);
			mWatchedBitmap = BitmapFactory.decodeResource(
					activity.getResources(), R.drawable.check_mark);
			setupIdleListener();
			...
			mList.setOnItemClickListener(new OnItemClickListener() {
				public void onItemClick(AdapterView<?> parent, View view,
					...
					final Movie movie = (Movie) mList.getAdapter().getItem(
							((FiveLabelsItemView) view).position);
			...
			mVideoManager.getMovies(new DataResponse<ArrayList<Movie>>() {
				public void run() {
					if (value.size() > 0) {
						...
						setTitle(actor.name + " - Movies");
						setNoDataMessage("No movies found.",
	...
	public void onContextItemSelected(MenuItem item) {
		final Movie movie = (Movie) mList.getAdapter().getItem(
				((FiveLabelsItemView) ((AdapterContextMenuInfo) item
		...
		case ITEM_CONTEXT_INFO:
			Intent nextActivity = new Intent(mActivity,
					MovieDetailsActivity.class);
			nextActivity.putExtra(ListController.EXTRA_MOVIE, movie);
			mActivity.startActivity(nextActivity);
			break;
	...
	public void onCreateOptionsMenu(Menu menu) {
		...
		sortMenu.add(2, MENU_SORT_BY_DATE_ADDED_ASC, 0,
				"by Date Added ascending");
		sortMenu.add(2, MENU_SORT_BY_DATE_ADDED_DESC, 0,
	...
	public void onOptionsItemSelected(MenuItem item) {
		...
		case MENU_SORT_BY_TITLE_ASC:
			...
			ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.TITLE);
			ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.ORDER_ASC);
			ed.commit();
			fetch();
			break;
		case MENU_SORT_BY_TITLE_DESC:
			...
			ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.TITLE);
			ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.ORDER_DESC);
			ed.commit();
			fetch();
			break;
		case MENU_SORT_BY_YEAR_ASC:
			ed = mActivity.getPreferences(Context.MODE_PRIVATE).edit();
			ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.YEAR);
			ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.ORDER_ASC);
			ed.commit();
			fetch();
		...
		case MENU_SORT_BY_RATING_ASC:
			ed = mActivity.getPreferences(Context.MODE_PRIVATE).edit();
			ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.RATING);
			ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.ORDER_ASC);
			ed.commit();
			fetch();
		...
		case MENU_SORT_BY_DATE_ADDED_ASC:
			ed = mActivity.getPreferences(Context.MODE_PRIVATE).edit();
			ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.DATE_ADDED);
			ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX
					+ AbstractManager.PREF_SORT_KEY_MOVIE, SortType.ORDER_ASC);
			ed.commit();
			fetch();
